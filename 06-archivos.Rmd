...
Lo anterior es un nuevo `data.frame` con dos variables, el continente y los conteos que estábamos buscando. Sin embargo, en las tablas de frecuencias suele ser útil agregar también una columna de frecuencias relativas o proporciones y de frecuencias acumuladas (si la variable fuese ordinal). Podemos hacer esto *mutando* el resultado anterior para generar las restantes columnas y lo guardamos en un nuevo `data.frame` llamado `frecuencias` para poder usarlo después:
...
Notemos que los objetos `minimo` y `maximo` son nuevos `data.frame`s, con tan sólo una observación. Podríamos usarlos para emitir algún mensaje con los hallazgos:
...
Por supuesto que si hubiesen empates en los valores máximos o mínimos tendríamos que pensar otra forma de comunicar el mensaje, pero con todos los conocimientos que tenemos de programación esa tarea es sencilla de resolver.
...
frecuencias %>% 
	rename(
		...
		`Frec. Absoluta` = n, 
		`Frec. Relativa` = frec_rel,
		`Frec. Abs. Acumulada` = frec_abs_acum,
		`Frec. Rel. Acumulada` = frec_rel_acum
	)

frecuencias
```

La función `count()` también permite obtener conteos asociados a cada combinación posible de las categorías de dos o más variables.

```{r}
count(datos, continente, campeon)
```

El resultado, como siempre, es un `data.frame` con una columna para cada una de las variables involucradas y otra para las frecuencias absolutas. Así, podemos ver que de todos los países americanos que jugaron algún mundial, 17 nunca fueron campeones y 3 sí.

Sin embargo, frecuencias como las anteriores suelen ser presentadas en **tablas de contingencia** o **tablas de doble entrada**, donde las filas se corresponden a las categorías de una variable y las columnas, a las de la otra. Existe un paquete que es muy útil para generar ese tipo de exploración de datos, llamado `janitor`. No forma parte del `tidyverse` pero está creado con el mismo diseño. Lo instalamos y cargamos:
...
Para crear una tabla de contingencia, usamos la función `tabyl()`:
...
Lo interesante de la función `tabyl()` del paquete `janitor` es que nos permite de manera muy sencilla agregar totales marginales:
...
Si analizamos una única variable categórica, la función `tabyl()` nos presenta tanto las frecuencias absolutas como las relativas:
...
tabla_gf <- 
	tabla_gf %>% 
	mutate(
		`Frec. Abs. Acumulada` = cumsum(n),        # cumsum = suma acumulada
