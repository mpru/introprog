...
Lo anterior es un nuevo `data.frame` con dos variables, el continente y los conteos que estábamos buscando. Sin embargo, en las tablas de frecuencias suele ser útil agregar también una columna de frecuencias relativas o proporciones, y de frecuencias acumuladas. Podemos hacer esto *mutando* el resultado anterior para generar las restantes columnas:

```{r}
count(datos, continente) %>% 
	mutate(
		frec_rel = n / sum(n),
		frec_abs_acum = cumsum(n), # cumsum = suma acumulada
		frec_rel_acum = cumsum(frec_rel)
	)
```

En lo anterior escribimos nombres para las columnas que son cómodos para programar. Si queremos cambiarlos para que se vean mejor en la salida, podemos usar `rename()`. Para poder usar nombres que contengan espacios, tenemos que encerrarlos en comillas invertidas (esto es poco práctico a la hora de escribir código, sólo sirve para mejorar la apariencia del resultado). Además, guardamos todo esto en un nuevo `data.frame` llamado `frecuencias` para poder usarlo después:
...
```{r}
ver <- filter(frecuencias, `Frec. Absoluta` == min(`Frec. Absoluta`))

cat("El continente ", ver$Continente, " con", ver$`Frec. Absoluta`)

frecuencias %>% 
	filter(`Frec. Absoluta` == min(`Frec. Absoluta`))

frecuencias %>% 
	slice(which.min(`Frec. Absoluta`))
```
