...
Generalmente los lenguajes de programación poseen un valor conocido como \texttt{NULO}, para representar un objeto vacío, sin información. El mismo suele emplearse como valor devuelto por funciones cuando no corresponde devolver otro tipo de resultado. En pseudocódigo podemos usar esta estrategia escribiendo \texttt{DEVOLVER\ NULO}, cuanod deseamos que nuestra función no devuelva nada. La representación en R de este tipo de objeto es \texttt{NULL}, que se trata de un objeto vacío que generalmente devuelven las funciones cuando el resultado es indefinido. Podemos crear funciones que terminen con un \texttt{return(NULL)}.

El objeto vación \texttt{NULL} no debe confundirse con otros dos valores existentes en el lenguaje R: \texttt{NA} y \texttt{NaN}:
...
\item
  \texttt{NaN} son las siglas de \emph{Not a Number} y es un valor numérico que generalmente surge como resultado de operaciones aritméticas imposibles de calcular, como indeterminaciones, raíces negativas, etc. (correr \texttt{0/0}, \texttt{log(-1)} o \texttt{sqrt(-1)} para verlo). Es un valor establecido por IEEE, el estándar con el cual se rige la representación numérica de los valores en la computadora.
\end{itemize}

Otro valor numérico muy especial es \texttt{Inf} (y su contrapartida negativa, \texttt{-Inf}), que es el resultado de almacenar un número muy grande o el resultado de una división por cero. No es semejante a \texttt{NA}, porque además de no ser de tipo lógico, no representa que hay un dato faltante sino que se trata de un valor numérico.
...
\caption{Ejemplo de un vector numérico}\label{fig:unnamed-chunk-102}
\end{figure}
...
\caption{Ejemplo de un vector numérico: índices para señalar cada posición.}\label{fig:unnamed-chunk-103}
\end{figure}
...
\caption{Ejemplo de un vector carácter y un vector lógico}\label{fig:unnamed-chunk-104}
\end{figure}
...
\caption{Creación de vectores en R}\label{fig:unnamed-chunk-106}
\end{figure}
...
Antes comentamos que en R los vectores se crean con expresiones como \texttt{x\ \textless{}-\ c(-4.5,\ 12,\ 2.71,\ -6,\ 25)}, donde sus elementos están listados de forma literal. También podemos crear vectores de un largo determinado dejando que cada posición quede ocupada por un valor por default. Por ejemplo, el siguiente código crea un vector tipo numérico con 10 posiciones, uno carácter con 7 y otro lógico con 2. En cada caso, R rellena por defecto todas las posiciones con el mismo valor: un 0 para todas las posiciones del vector numérico, un carácter vacío \texttt{""} para las del vector carácter y el valor \texttt{FALSE} para el vector lógico:
...
\caption{Ejemplo de una matriz numérica}\label{fig:unnamed-chunk-113}
\end{figure}
...
\caption{Ejemplo de una matriz numérica: índices para señalar cada posición}\label{fig:unnamed-chunk-114}
\end{figure}
...
\caption{Recorrer una matriz por fila}\label{fig:unnamed-chunk-119}
\end{figure}
...
\caption{Recorrer una matriz por columna}\label{fig:unnamed-chunk-121}
\end{figure}
...
\caption{Matriz traspuesta}\label{fig:unnamed-chunk-123}
\end{figure}
...
\caption{Ejemplo de una lista}\label{fig:unnamed-chunk-128}
\end{figure}
...
\caption{La lista en la pestaña Environment de RStudio}\label{fig:unnamed-chunk-130}
\end{figure}
...
\caption{Ejemplo de una lista}\label{fig:unnamed-chunk-131}
\end{figure}
...
Para ejemplificar, recordemos el siguiente ejercicio de la práctica 3: escribir el algoritmo y el programa en R para la creación de la función \texttt{triangulos(a,\ b,\ c)} que a partir de la longitud de los tres lados de un triángulo \texttt{a}, \texttt{b} y \texttt{c} (valores positivos) lo clasifica con los siguientes resultados posibles: no forman un triángulo (un lado mayor que la suma de los otros dos), triángulo equilátero, isósceles o escaleno. Vamos a modificar la función para que tenga el siguiente comportamiento: la función debe devolver el tipo de triángulo como cadena de texto y el valor numérico del perímetro del mismo (o un 0 si no es triángulo). Es decir, la función debe devolver tanto un objeto de tipo carácter y otro de tipo numérico. Para lograrlo los encerraremos en una lista:
...
\caption{Primeros registros del archivo reservas.txt}\label{fig:unnamed-chunk-145}
\end{figure}
...
\caption{Bosquejo de las estructuras de datos necesarias}\label{fig:unnamed-chunk-151}
\end{figure}
...
\caption{Abrir la terminal en Windows. En computadoras con Windows en español, en lugar de Command Prompt dice Símbolo del sistema.}\label{fig:unnamed-chunk-171}
\end{figure}
...
\caption{Línea de comandos de Windows.}\label{fig:unnamed-chunk-172}
\end{figure}
...
\caption{Contenido del directorio actual (Windows).}\label{fig:unnamed-chunk-173}
\end{figure}
...
\caption{Carpeta en la cual deseamos trabajar.}\label{fig:unnamed-chunk-174}
\end{figure}
...
\caption{Cambio de directorio de trabajo y listado de archivos en el mismo.}\label{fig:unnamed-chunk-175}
\end{figure}
...
\caption{Abrir la terminal desde una carpeta en particular en Windows.}\label{fig:unnamed-chunk-176}
\end{figure}
...
\caption{Correr el programa de R desde la consola en Windows.}\label{fig:unnamed-chunk-178}
\end{figure}
...
\caption{Programa paridad.R.}\label{fig:unnamed-chunk-180}
\end{figure}
...
\caption{Programa salario.R.}\label{fig:unnamed-chunk-182}
\end{figure}
...
\caption{Programa suma.R.}\label{fig:unnamed-chunk-184}
\end{figure}
